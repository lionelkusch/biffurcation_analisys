function res = plotcycle_modify_stability(x,h,s,e,increment)
%
...
if length(e)==2
 ...
 %   pause(0.1);
    for j=1:increment:size(x,2)
        ...
        plot(x((0:lds.tps-1)*lds.nphase+1,j),x((0:lds.tps-1)*lds.nphase+2,j),strcat(color,'.-'));
    end
    for j=[s.index]
  ...
  %  end
    ...
    for j=1:increment:size(x,2)
        ...
        plot3(x(e(1),j)*ones(lds.tps,1),x((0:lds.tps-1)*lds.nphase+e(2),j),x((0:lds.tps-1)*lds.nphase+e(3),j),strcat(color,'.-'));
    end
    for j=[s.index]
